...
public class C_Scanner {
	public void inputTest1() {
		...
		// 2. next() : 사용자가 입력한 값을 "공백"을 기준으로 "한 단어"만 가져오는 메서드
		// 서[\s]혜림[\n]
		String lastname = sc.next(); // 서
		String firstname = sc.nextLine(); // [스페이스]혜림
		// nextLine함수는 호출 후 "버퍼"의 값을 비워준다 => [엔터]값 소멸
		System.out.print("당신의 나이는 몇 살 입니까 : ");
		int age = sc.nextInt(); // 입력버퍼에 있는 다음값을 Int 자료형으로 뽑아쓰겠다
		...
		System.out.println("당신의 이름 : "+lastname+firstname);
	...
	public void inputTest2() {
		...
		// 문자를 뽑는 메서드는 없다 (sc.nextChar X)
		// 문자를 뽑기 위해선, 문자열의 메서드를 이용해야함
		
		System.out.print("이름 : ");
		String name = sc.nextLine();
		
		// (char)강제형변환은 기본자료형 끼리만 가능 (문자는 참조자료형이기 때문에 불가)
...
//		char b = str.charAt(1);
		 ...
		 * sc.nextLine()을 한번 더 호출하여 버퍼(에 들어있는 엔터값)를 비워줘야 한다
		 * 2) 스캐너를 통해 '문자'값을 입력 받아야 될 경우
		 * sc.nextLine()을 통해 "문자열"을 입력받고, 그 후 charAt(index)를 호출한다
		 */
	}
