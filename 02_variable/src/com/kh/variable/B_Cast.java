...
public class B_Cast {
	 ...
	 * 		작은 바이트의 자료형 -> 큰 바이트의 자료형으로 변환될 때
	 * 2. 강제(명시)형변환 => 자동형변환이 불가능한 경우, 내가 직접 형변환 연산자를 사용하여 변환한다.
	 * 		큰 바이트의 자료형 -> 작은 바이트의 자료형으로 변환하고 싶을 때
	 * 		형변환 연산자 ==> (바꿀 자료형) 값
	 */
	...
	public void autoCasting() {
		...
		int i3 = 1000;
		long l3 = /*(long)*/ i3;
		long l4 = /*(long)*/ 2000; //int 리터럴을 롱 자료형으로 넣는다
		// 다만 너무 큰 리터럴 값을 사용해야 하는 경우 l을 붙여줌
		
		// 특이케이스 3. long -> float
		// long ==> 8byte, float ==> 4byte
		// 정수가 실수에 담길 때에는 큰 사이즈의 정수여도 작인 사이즈의 실수에 대입 가능
		// 실수는 지수표기법을 사용하기 때문에 long자료형보다 담을 수 있는 값의 범위가 훨씬 크다
		long l5 = 10000000000L;
		float f5 = /*(float)*/ l5;
		
		// 특이케이스 4. char <-> int
		// 양방향 형변환 가능 (단, int가 양의 정수일 때)
		char ch = /*(char)*/ 65; // 아스키 코드표의 범위(0~127)
		System.out.println("ch : "+ch);
		
		int num = /*(int)*/ '서'; // 한글과 같은 기타 나라의 언어는 유니코드표(0~65xxx)에 기술되어 있다.
		System.out.println(num);
		
		// 특이케이스 5. byte와 short 연산
		byte b1 = 127;
		byte b2 = 127;
		//byte나 short로 연산시 연산결과는 항상 int 자료형으로 취급된다.
