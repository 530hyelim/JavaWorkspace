...
public class Run {
	public static void main(String[] args) {
		...
		try {
			...
			// 2. 경로를 지정한 파일명을 제시하여 파일객체 생성하기 (절대경로)
			// -> 해당 경로가 반드시 존재하는지 확인 후, 객체를 생성해야 한다
			...
			if(!isExist) {
				bbbFolder.mkdir(); // 단일 디렉토리. 폴더가 여러개일경우 mkdirs()
			}
			...
			System.out.println(file3.isDirectory()); // 파일이므로 false
			
			System.out.println("파일명 : "+file3.getName());
			System.out.println("상위폴더 : "+file3.getParent());
			System.out.println("파일용량 : "+file3.length()+"byte");
		 ...
		 * 스트림의 구분
		 *   - 데이터의 크기에 따른 분류
		 *     1) 바이트 스트림 (바이트 단위로 송수신)
		 *       - 모든 바이트 스트림은 InputStream/OutputStream 추상클래스를 상속받아 구현
		 *       - 읽기는(read()), 쓰기는(write())메서드를 사용
		 *       - 1byte짜리 데이터가 오고갈 수 있는 스트림
		 *       
		 *     2) 문자스트림 (캐릭터 단위로 송수신)
		 *       - 모든 문자스트림은 Reader/Writer 추상클래스를 상속받아 구현
		 *       - 읽기는(read()), 쓰기는(write())메서드를 사용
		 *       - 2byte단위로 데이터가 오고가는 스트림
		 *       
		 *   - 외부매체와의 연결 가능 여부
		 *     1) 기반 스트림 : 외부매체와 직접연결이 가능한 스트림 (e.g. System.in)
		 *     2) 보조 스트림 : 기반스트림의 부족한 기능을 보조하기 위한 스트림 (e.g. Buffered~)
		 *     		(속도 향상, 자료형 변환, 객체단위 입출력, ...)
		 *     		단독으로 사용이 불가능하며, 기반스트림이 필수
		 *   		new BufferedReader(new InputStreamReader(System.in))
		 *   		System.in : 기반스트림
		 *   		new InputStreamReader : 보조스트림
		 *   		new BufferedReader : 보조스트림 => 데코레이트 패턴!
		 */
	}
}
