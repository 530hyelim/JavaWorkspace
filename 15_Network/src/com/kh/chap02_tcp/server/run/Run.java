...
public class Run {
	public static void main(String[] args) {
		 ...
		 * 	 - 서버와 클라이언트간의 연결이 되어있어야 데이터를 교환한다.
		 * 	   (서버가 먼저 연결요청을 받을 준비가 되어 있어야 함)
		 * 	 - 신뢰성있고 안전한 데이터 전달이 가능
		 * 	 - 클라이언트와 서버간에 연결이 되어 있으므로 "실시간"으로 데이터 송수신이 가능
		 * 
		 * Socket
		 * 	 - 클라이언트와 서버간에 실시간 "통신"에 사용되는 객체
		 * 	 - 소켓이 소유한 ip번호+port번호(=endpoint)를 통해 소켓을 식별한다
		 * 
		 * ServerSocket
		...
		try {
			...
			while(true) {
				...
				Socket client = server.accept(); // 여기서 스레드가 멈춤. 클라이언트가 접속 요청 넣을 때까지.
				System.out.println("### 클라이언트 연결 !!! ###");
				
				// 4) 연결된 클라이언트와 통신할 수 있는 입출력 (바이트)스트림 생성
				BufferedReader br = new BufferedReader(new InputStreamReader(client.getInputStream()));
				PrintWriter pw = new PrintWriter(client.getOutputStream(), true); // autoflush = true
